...
3. **Streams** `quote`/`trade`/`midpoint`/`depth`/`book`/`order_book` messages as they arrive, fanned out to all
   connected consumers.
...
| `book`       | A batch of incremental **price-aggregated** order-book changes. |
| `order_book` | The same batch shape, **order-level**: each change is one order's state, keyed by the venue's `order_id`. |
...
> top-*N* `depth`, and the order-level [`order_book`](#book) carrying the venue's own `order_id`. Neither
...
| `clear_reason` | uint8 | Why the **venue** withdrew the book. **Optional** — absent, never `0`, unless this batch carries a clear the venue itself sent. `book` only. See *Why the venue cleared* below. |
...
**Why the venue cleared: `clear_reason`.** A clear can come from either end. Most do not come from the venue at all — the producer emits one whenever it has to re-baseline you (a failover between upstream publishers, a recovery, the connect-time bootstrap), and those say nothing about the instrument. `clear_reason` is present **only** when the upstream venue itself withdrew the book and named a cause, so its absence means the venue said nothing, and it is absent rather than `0` because `0` is a cause a venue can actually give.

| Value | Name | Meaning |
|---|---|---|
| 0 | Unspecified | Cleared, no reason given. |
| 1 | Halt | Instrument halted; the book is withdrawn. |
| 2 | SessionEnd | The trading session closed. |
| 3 | VenueReset | The venue cleared the book. |
| 4 | Settled | The instrument settled or expired. The book is **permanently** gone. |
| 255 | Other | Venue-specific; documented out of band. |

Three rules. **Treat an unrecognized value as unknown, not as `Other`** — the field is passed through byte-for-byte, the range `5..=254` is where a later revision's causes land, and `255` is a claim in its own right rather than that range's name. **It describes the batch, not one change**: a clear bounded by a price is re-served as the exact `delete`s it removed and carries no `clear` entry to hang the cause on, so the field sits on the message; where one batch coalesces two of the venue's clears for a market, it carries the later one's cause. **It is only ever on `book`** — the upstream Market-by-Order feed has no equivalent field, so `order_book` never carries one, and its absence there is not a statement that the market is live.

Finally, the producer **passes this through and does not act on it**: a `Settled` batch is still followed by whatever the venue sends next, and the instrument stays in the catalog and keeps its `instrument` definition. What a consumer does with the distinction — retiring a settled market, holding a halted one inactive — is the consumer's decision.
...
- This document defines **v1**, which includes: the `instrument`/`quote`/`trade`/`midpoint`/`depth`/
  `book`/`order_book` data messages, the venue-level `status` feed-health message, optional **subscribe/unsubscribe**
