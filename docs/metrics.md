...
| `dz_mbp_book_clears_total` | counter | `venue`, `reason` | `BookClear`s as they arrive, by the venue's own `Clear Reason` (`unspecified`/`halt`/`session_end`/`venue_reset`/`settled`/`other`, plus `unknown` for the spec's unassigned range). The **conformance census**: acting on that field — retiring a settled instrument, holding a halted one inactive — is only worth building once the publishers are seen to populate it, and a publisher that reports every departure under one cause shows up here as a single series. Counted on arrival, *before* the sequence check, so a clear the book declines still reports what the publisher said; it therefore does not track `dz_book_dropped_total` or the published batches. The label set is bounded on purpose — the raw byte is on the wire as `book`'s `clear_reason`, which is where a consumer that needs it reads it. |
