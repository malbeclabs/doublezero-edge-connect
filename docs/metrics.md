...
| `dz_mbo_forced_rebaselines_total` | counter | `venue`, `reason` | Markets withheld and re-baselined because the cross-publisher guard could not answer. `reason="disagreement"`: two publishers claimed different resting state for one order, so neither is known to be right — publishing the larger rewinds a consumer past a fill the venue already applied, the smaller lets a forged size mute a real order. `reason="guard_evicted"`: the per-market cap (1024 orders) aged out an order a peer's copy could still have been racing, reopening the resurrection path. An eviction *past* that horizon costs the guard nothing and is not counted, so a large but unhurried book streams normally; this fires when 1024 distinct orders churn inside `--arb-book-dedup-window-ms`, which is the case where the cap is genuinely too small for the venue. Each one costs a full re-publish of the market's book — serialized to every client, and O(book) inside the shared arbiter mutex, so it stalls every ingest receiver on every feed rather than merely spending bandwidth. Rate-limited to one per market per `--arb-book-dedup-window-ms` for that reason, since a single datagram can raise the flag. ⚠️ The market keeps withholding meanwhile and those batches are **lost, not delayed** — they never reach the map the re-publish is built from, so a consumer's book skips them until some publisher sends a re-baseline of its own. A sustained rate on either reason is the signal to reconsider the per-publisher book model. The re-publish is the book **the wire agreed on**, never one publisher's own — an arm's private accumulator is whatever that source sent, and republishing it as full state on an unauthenticated wire would make one forged datagram enough to replace a market's book. |
