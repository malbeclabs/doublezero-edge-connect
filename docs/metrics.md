...
| `dz_book_events_abandoned_total` | counter | `venue` | Logical events whose buffered changes outgrew the replay accumulator's cap (8,192) before their `last` arrived. The market is then withheld from every bootstrap — connect, `subscribe` and lag repair — until a producer re-baseline, so a non-zero rate is markets missing from new clients rather than merely a wide event. |
