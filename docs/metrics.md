...
| `dz_mbo_guarded_tombstones` | gauge | — | Removed orders the resurrection guard holds across every tracked market, against a process-wide ceiling of 1,048,576. Sized by how far the publishers lag each other (removal rate × lag), so it is flat and small when they are in step and rises with separation. ⚠️ **It is an aggregate, and the cap that fires first is per market — 65,536, six per cent of this figure.** A single market walking to its own cap and blacking out therefore reads as flat headroom here; alert on `dz_mbo_market_invalidations_total`, which is the event itself, and treat this gauge as the process-wide backstop rather than as per-market headroom. Either cap invalidates markets, and the process-wide one is charged to the market holding the most, never to whichever market records the next removal. |
