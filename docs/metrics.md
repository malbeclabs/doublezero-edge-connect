...
| `dz_mbp_reanchor_total` | counter | `venue`, `reason` | Books forced out of `Ready` **without installing anything**. `short_book`: a declined complete rotation declared materially more levels than the book holds (the shortfall exceeded both 8 levels and 10% of the declared total), so the short install it was serving is dropped and the *next* rotation installs whole — the market is held by its peer path for one rotation meanwhile, and one `dz_path_authority_transfers_total{reason="health"}` pair goes with each. Expect zero in steady state; a burst at a publisher restart is the fix working, and a market re-anchoring every 60 s (the per-book interval guard) is a publisher whose declared total never matches what it sends. |
