...
| `dz_book_resurrections_dropped_total` | counter | `venue` | Order-level changes dropped because the market had already published that order as gone. A venue never reuses an order id, so each one is a lagging publisher's stale copy refused before it could resurrect a dead order in every consumer's book. **This is the guard order-level racing rests on, and a non-zero rate is it working, not a fault** — it rises with how far one publisher lags its peers. The guard's reach is an order count (the same per-market cap the window uses) rather than a clock, which is what keeps `--arb-book-dedup-window-ms` a cost knob; what happens when that count binds is `dz_mbo_forced_rebaselines_total`. |
| `dz_mbo_forced_rebaselines_total` | counter | `venue`, `reason` | Markets withheld and re-baselined because the cross-publisher guard could not answer. `reason="disagreement"`: two publishers claimed different resting state for one order, so neither is known to be right — publishing the larger rewinds a consumer past a fill the venue already applied, the smaller lets a forged size mute a real order. `reason="guard_evicted"`: the per-market cap on tracked *dead* orders (512) aged out an order a peer's copy could still have been racing, reopening the resurrection path. A tombstone every serving arm has already reported costs the guard nothing to drop and is not counted, so an ordinary busy market streams normally; this fires when 512 removals a peer never mirrored accumulate on one market, which is the case where the cap is genuinely too small for the venue. Note the trigger is a **removal count, not window-scoped churn** — tombstones are deliberately never expired by `--arb-book-dedup-window-ms`, so shrinking that knob does not reduce these. Each one costs a full re-publish of the market's book — serialized to every client, and O(book) inside the shared arbiter mutex, so it stalls every ingest receiver on every feed rather than merely spending bandwidth. Rate-limited to one per market per `--arb-book-dedup-window-ms` for that reason, since a single datagram can raise the flag. ⚠️ The market keeps withholding meanwhile and those batches are **lost, not delayed** — they never reach the map the re-publish is built from, so a consumer's book skips them until some publisher sends a re-baseline of its own. A sustained rate on either reason is the signal to reconsider the per-publisher book model. The re-publish is the book **the wire agreed on**, never one publisher's own — an arm's private accumulator is whatever that source sent, and republishing it as full state on an unauthenticated wire would make one forged datagram enough to replace a market's book. |
