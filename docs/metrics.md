...
| `dz_mbp_book_buffer_overflows_total` | counter | `venue` | One book's *own* delta buffer overflowed: it held `MAX_BUFFERED_DELTAS` (2^18) deltas without a snapshot arriving to anchor them. The buffer is dropped and the book marked `Gap`, so the market fails over to a peer — deltas were lost, and a book assembling a rotation over live levels would otherwise keep being reported as healthy while it publishes nothing. Distinct from the cross-instrument budget above: that is the host's memory ceiling across every book, this is one instrument whose rotation never came. |
...
| `dz_mbp_declined_rotation_shortfall_levels` | histogram | `venue` | Levels a declined **complete** rotation (`depth_bound == 0`) declared above what the book holds. The only readings that should exist are near zero: a rotation is captured a few hundred milliseconds before it reaches us and our book has applied deltas since, which measures 1–3% on a ~550-level market. Completeness is the publisher's own claim, so a population drifting up the buckets is a publisher shipping short complete-flagged groups — visible here before any of them crosses the re-anchor bound. Read against the next row. |
| `dz_mbp_reanchor_total` | counter | `venue`, `reason` | Books forced out of `Ready` **without installing anything**. `short_book`: **two consecutive** declined complete rotations declared materially more levels than the book holds (the shortfall exceeded both 8 levels and 10% of the declared total), so the short install it was serving is dropped and the *next* rotation installs whole. Two are required because one cannot be told from a book that legitimately shrank since that rotation was captured, and "two" means two complete rotations with no agreeing one in between — nothing expires a first strike by elapsed time. Reading it against the histogram: a **sustained** top-bucket population with this counter flat is the false positive being suppressed. A transient one is not, since the same shape is produced by a first strike still waiting for its confirmation and by a confirmed pair held off by the 60 s interval guard — the market is held by its peer path for one rotation meanwhile, and one `dz_path_authority_transfers_total{reason="health"}` pair goes with each. Expect zero in steady state; a burst at a publisher restart is the fix working, and a market re-anchoring every 60 s (the per-book interval guard) is a publisher whose declared total never matches what it sends. |
| `dz_mbp_snapshot_installs_total` | counter | `venue`, `completeness` | Snapshot groups that installed, by whether the publisher declared the book `complete` (`depth_bound == 0`) or `bounded`. The denominator for the two rows above and the one below. |
| `dz_mbp_install_shrank_book_total` | counter | `venue` | Installs that left the book with fewer levels than it held. Zero by construction on a cold rebuild (the book was empty), so this is a rotation replacing a **live** book with a smaller one — the other way a market can end up short, and the one a re-anchor does not catch. |
| `dz_mbp_book_clears_total` | counter | `venue`, `scope` | Wire `BookClear`s the book applied, by `scope` (`entire_side`/`from_price`). A whole-side clear the publisher never repopulates leaves the same shape as a short install, so the two need telling apart. |
| `dz_mbp_snapshot_levels_dropped_total` | counter | `venue`, `reason` | Levels of a snapshot group the processor deliberately did not route at all, by why: `reset` (an `InstrumentReset` killed the anchor it was assembling against, so the route went with the group), `stale_era` (its `Reset Count` is the publisher's previous run, still draining off the snapshot socket after a restart — refused rather than installed, so the dead session's book is never republished), `no_definition` (its instrument's definition had not resolved yet — a cold-start transient of tens of thousands of levels that clears as reference data fills the catalog), `end_of_session` (the session the group belonged to ended, along with that path and channel's books). All four are correct behaviour with a permanent floor on a live feed; they are counted here so the orphan series above stays alertable. A group's tombstone is spent by the next `SnapshotBegin` on its channel, so a level arriving outside any group's stream still reaches the orphan series. |
...
| `dz_mbp_slot_regressions_total` | counter | `venue` | `BatchBoundary` naming a slot **below** the one its channel had already committed within one publisher era. It is ignored — the slot does not move and no event closes on it, since closing on a superseded boundary commits every buffering consumer mid-event. (A boundary naming the slot *exactly* is an ordinary redelivered datagram, ignored the same way and not counted.) Expect zero. Read it against two series to place the cause: with `dz_seq_events_total{kind="stale"}` rising it is reordering the sequence check already drops; with `stale` flat and `dz_path_authority_transfers_total` flat the boundary stream of one path named a slot it had passed, which is upstream. Observed once, after a 1.5 s pause in what both publishers emitted. |
| `dz_mbp_closes_refused_total` | counter | `venue` | Closing batches a publish gate refused — the book is mid-rotation, gapped, or not yet revealed. The instrument stays owed its close and is retried at the next boundary rather than forgotten, so this is the observable for "a path holds markets it cannot publish"; it should be rare and self-clearing. |
| `dz_book_events_abandoned_total` | counter | `venue` | Logical events whose buffered changes outgrew an accumulator's cap (8,192) before their `last` arrived. Counted per accumulator: one event on a mirrored market scores once per path that carried it plus once for the replay map (three on Phoenix). The market is withheld from every bootstrap until the serving path republishes its whole book at the event's close — about one slot — so each one costs connected consumers one extra `Clear`-led re-baseline. |
| `dz_book_bootstrap_lost_total` | counter | `venue`, `category`, `channel`, `instrument_id` | Times a market's replay entry stopped being a complete book, so new WebSocket subscribers stopped being bootstrapped with it. The only market-labelled series: join `channel`/`instrument_id` to a symbol through `/v1/products`. Causes: an event past the cap above, a serving-path transfer onto a path whose own accumulator is incomplete, or a market's state being evicted or reset. Past 4,096 distinct markets over the process's life, further ones share `channel="other", instrument_id="other"`. |
| `dz_book_bootstrap_withheld` | gauge | `venue`, `category`, `channel`, `instrument_id` | Markets withheld from new subscribers' bootstraps right now: `1` while a market lasts; the series is removed once it completes again or its state is dropped. Past 1,024 markets withheld at once, further ones share `channel="other", instrument_id="other"`. Worth an alert on any series holding `1` for more than a few seconds — recovery is normally one slot, and the same state is readable without the metrics endpoint as `book_complete: false` on `/v1/products`. |
...
| `dz_path_authority_transfers_total` | counter | `venue`, `reason`, `rebaselined` | Authority handovers, by `reason` (`initial`/`health`/`health_revert`/`silence`/`margin`) and by whether the handover obliged a consumer re-baseline (`rebaselined=yes`/`no`). ⚠️ **`health` is the failover path and `silence` is not.** Health is per market and fires on the peer's very next batch, so a restarting publisher's markets move in a second or two; silence is per path and requires the leader to stop *entirely* for `--arb-leader-timeout-secs`, which a publisher trickling a few datagrams a second through a restart never does. A long gap before a `silence` transfer is that publisher's ramp, not a bridge timer, and the markets were already being served by the peer through health. ⚠️ **It is not the re-baseline volume — `dz_emit_total{kind="book"}` is.** Two classes of consumer re-baseline are still uncounted here: the first market to speak after a `margin` transfer, and every forced re-baseline discharge (`dz_mbo_forced_rebaselines_total`, both reasons), where the market never changed hands. On the venue-wide reasons the label describes the transfer rather than one market: `margin` is recorded `yes` because it re-baselines every market the new leader was not already serving. `health` is a single market moving to a peer and `health_revert` the elected path taking it back once its own book recovers — the two halves of one episode, so read them as a pair: `health` without `health_revert` is a path that has not come back. Both are per market; the other three reasons are venue-wide. A sustained `rebaselined="yes"` rate means the thresholds are too loose, since each one costs the consumer a full book. The label is what a handover that costs nothing is told apart by; `rebaselined` says the handover obliges a re-baseline, not that the batch carrying the count delivered it — one waiting for its new path's `last` is discharged by a later batch. |
...
| `dz_path_markets_held` | gauge | `venue`, `path` | Markets each path is currently **serving** — the venue's elected path, plus any market a health override moved to a peer. ⚠️ **It is a periodic sample, not a live view**: computing it is O(markets + paths) under the shared arbiter mutex, so it is refreshed only on the sampler tick (`--arb-sample-interval-secs`, 300 s by default). A health override moves markets within a second or two of a path going unhealthy, so for up to one tick afterwards this gauge still names the path that was serving before — which reads, wrongly, as the gauge naming the leader. Read a failover off `dz_path_authority_transfers_total` and the per-transfer log, which are immediate. `path` is a stable per-venue ordinal (`path0`…`path7`, then `other`), never the spoofable source IP address. All markets on one path is the steady state; a persistent split means health overrides are fragmenting the venue, i.e. the elected path's books keep gapping. |
...
| `dz_mbo_forced_rebaselines_total` | counter | `venue`, `reason` | Markets withheld and re-baselined because a gate could not carry the consumer's book forward (the `mbo_` in the name is historical; `reason="dropped_batch"` comes from the price-aggregated single-path gate, where it counts a batch dropped from the path the consumer was following — that book now has a hole only a re-baseline closes, and with a quiet peer nothing else would ever raise one). `reason="disagreement"`: two publishers claimed different resting state for one order, so neither is known to be right — publishing the larger rewinds a consumer past a fill the venue already applied, the smaller lets a forged size mute a real order. Each one costs a full re-publish of the market's book — serialized to every client, and O(book) inside the shared arbiter mutex, so it stalls every ingest receiver on every feed rather than merely spending bandwidth. Counts **markets**, not datagrams: raising the flag is idempotent, so a market whose peer is quiet scores once per episode however many batches are dropped meanwhile. The 250 ms floor between discharges applies to the order-level racer (`reason="disagreement"`) alone; the single-path gate discharges on the serving path's next terminated event. That interval is a fixed constant and deliberately **not** `--arb-book-dedup-window-ms`, which it once shared: widening the window for its dedup reach would otherwise lengthen how much of a real disagreement's feed is skipped. ⚠️ The market keeps withholding meanwhile and those batches are **lost, not delayed** — they never reach the map the re-publish is built from, so a consumer's book skips them until some publisher sends a re-baseline of its own. A sustained rate is the signal to reconsider the per-publisher book model. The re-publish is the book **the wire agreed on**, never one publisher's own — a path's private accumulator is whatever that publisher sent, and republishing it as full state on an unauthenticated wire would make one forged datagram enough to replace a market's book. |
...
| `dz_ws_lag_repairs_total` | counter | — | Book-repair passes run for a lagging client. Counts the **pass**, not the re-baselines it wrote: a pass whose filters match no baselined market — every pass, on a deployment carrying no book-bearing feed — sends nothing and still counts, because what this series is read against is the pace, not the traffic. Paced per client (5s), so it sits **below** `dz_ws_client_lagged_total` whenever a client lags faster than it can be repaired; the difference is lag events coalesced into a later repair, not lost ones. Only `book`/`order_book` are repaired — `quote`/`depth` are full state and `instrument` returns on the next refdata burst — so a lag costs O(markets), never O(catalog) — and nothing at all for a client whose `type` filter excludes both book types, which is not counted here at all. |
| `dz_ws_bootstrap_withheld_total` | counter | `venue`, `release` | Market bootstraps a client's join withheld because the market was mid-event, by how the wait ended: `complete` (its event closed and the client was bootstrapped — the normal path, sub-second) or `deadline` (it did not within 5s, so the client was bootstrapped from the last complete state anyway and the open event's changes are lost to it). Steady `deadline` on one venue means its channel has stopped closing events, not that clients are joining badly. A market with no complete book anywhere in this process is never released and never counted here. |
| `dz_ws_frames_dropped_total` | counter | `venue`, `reason` | Book frames not forwarded to a client: `watermark` (the batch is already in the bootstrap that client was just handed — expected, a small per-join prefix) or `awaiting` (the market is still withheld). `awaiting` that does not stop is a client dark on that book, which is what this series exists to show; read it against `dz_ws_bootstrap_withheld_total`. |
