...
| `dz_mbo_arm_disagreement_total` | counter | `venue` | Two publishers reported **more** resting quantity for one order than a peer had already reported for it. A resting order only ever shrinks, so this is a publisher that missed a fill and believes its book is still contiguous — silent drift, made observable. **Any sustained rate is a correctness alarm**, and the trigger to reconsider deriving output from per-publisher books at all (dedup on input, into one shared book, makes the divergence structurally impossible). An ordinary interleaved race, where an arm delivers a smaller remainder first, is not counted. |
| `dz_mbo_removed_evicted_total` | counter | — | Removed order ids a book forgot on reaching its resurrection-guard cap (2^20 per book). That guard is what makes order-level racing safe — a removed id is never re-added, however late a copy arrives — so eviction reopens the path for a duplicate arriving later than that many removals on one instrument. Should stay flat at zero; sustained non-zero means the cap is too small for the venue's churn. |
...
`--arb-book-dedup-window-ms` (250) governs the *other* gate — how long a delivered order-level book event is remembered so a slower publisher's copy is recognized as a duplicate. It is not a re-election tunable and it is **not** a correctness parameter: a removed order id is never re-added regardless of the window, so an undersized value costs a redundant emission and a wasted apply, not a corrupt book. Read it against `dz_book_events_deduped_total`.
