...
| `dz_mbp_slot_regressions_total` | counter | `venue` | `BatchBoundary` naming a slot the channel had already committed within one publisher era. PROTOCOL.md promises a consumer's `batch_id` never moves backwards, so the high-water stands and the field goes **absent** until the publisher passes it — carrying the high-water forward instead would claim a slot the batch's content is older than. Expect zero. Read it against two series to place the cause: with `dz_seq_events_total{kind="stale"}` rising it is reordering the sequence check already drops; with `stale` flat and `dz_path_authority_transfers_total` flat the boundary stream of one path named a slot it had passed, which is upstream. Observed once, after a 1.5 s pause in what both publishers emitted. |
