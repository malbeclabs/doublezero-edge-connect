...
| `dz_ws_lag_rebaselines_total` | counter | — | Book re-baselines sent to repair a lagging client. Paced per client (5s), so it sits **below** `dz_ws_client_lagged_total` whenever a client lags faster than it can be repaired; the difference is lag events coalesced into a later repair, not lost ones. Only `book`/`order_book` are repaired — `quote`/`depth` are full state and `instrument` returns on the next refdata burst — so a lag costs O(markets), never O(catalog) — and nothing at all for a client whose `type` filter excludes both book types. |
