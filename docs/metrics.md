...
| `dz_mbo_forced_rebaselines_total` | counter | `venue`, `reason` | Markets withheld and re-baselined because the cross-publisher guard could not answer. `reason="disagreement"`: two publishers claimed different resting state for one order, so neither is known to be right — publishing the larger rewinds a consumer past a fill the venue already applied, the smaller lets a forged size mute a real order. `reason="guard_evicted"`: a tracked order aged out at the per-market cap (1024), reopening the resurrection path. Each one costs a full re-publish of the market's book, so **the rate is the cost of the per-market cap being small**: the cap is sized against its product with the 16,384-market ceiling, and a book larger than it re-baselines roughly once per 1024 order events. A sustained rate on either reason is the signal to reconsider the per-publisher book model. |
