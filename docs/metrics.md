...
| `dz_trades_dropped_total` | counter | `venue` | Trades collapsed before the wire: a duplicate `trade_id` still inside the window, or — on a `Sticky` venue — a non-serving arm's copy dropped by the per-venue tape gate. On such a venue the steady-state rate is the challenger arm's whole print stream, so read it against `dz_emit_total{kind="trade"}` rather than as a fault. |
...
| `dz_tape_arm_transfers_total` | counter | `venue` | The same move one level down: which **arm** of the owning row serves the tape, for `Sticky` venues. Transfers on the book election naming a different arm, or on the incumbent going silent for 5s. Its steady state is zero; a sustained rate means the two arms are trading the tape, and the same double-or-drop window applies. |
...
**The `dz_arm_*` and `dz_book_*` series emit only once a venue actually serves `book`.** The gate and the Market-by-Price processor's health reporting both have callers now, but the only `FEEDS` row of that kind (`lashay-2`) stays inert until the group rename lands, so no venue elects an arm and a scrape today still reports nothing for them; do not read an empty result as a healthy venue.
