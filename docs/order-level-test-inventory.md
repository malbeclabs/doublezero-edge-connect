...
- `a_stream_of_in_bound_stamps_cannot_ratchet_the_frontier_past_the_host_clock` (in `tests/order_level_consumer_book.rs`, consumer-visible) — the per-step forward bound caps one advance, not a stream of them; without the host-clock anchor a spoofed publisher strands the whole channel and empties the guard.
