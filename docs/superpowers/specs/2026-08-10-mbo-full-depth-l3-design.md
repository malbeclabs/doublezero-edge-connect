...
**The residual correctness risk is a drifted publisher winning a race**, described under *Racing mode* above and detected by `dz_mbo_arm_disagreement_total`. It is bounded to the pre-existing `Ready`-wedge condition rather than being a new failure mode, and the shared-book model remains the fallback if it proves real.
