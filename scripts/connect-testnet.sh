...
# The management hints close the run, and the tunnel's state is what the operator has
# to act on -- so the last line restates it rather than reading as a clean finish.
if [ "$CONNECT_STATE" != connected ]; then
  echo
