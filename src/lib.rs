...
//! and types directly — e.g. `examples/pcap2datagrams.rs`, which decodes captured datagrams
//! through the same `ingest::codec` the bridge uses (so the converter doubles as a codec-offset
//! validator against the live feed).
