...
mod tests {
    ...
    fn the_output_tail_is_bounded_and_char_safe() {
        ...
        // A 3-byte char is what actually exercises the boundary snap: the cut sits at
        // `len - MAX_CLI_DETAIL`, and with an even-byte char both are even, so it lands on a
        // boundary and the snap is dead code the test can't tell is missing. `€` puts the cut one
        // byte inside a sequence, so deleting the snap panics here instead of passing.
        let long = "€".repeat(MAX_CLI_DETAIL);
        assert!(
            !long.is_char_boundary(long.len() - MAX_CLI_DETAIL),
            "the fixture must cut mid-character or it proves nothing"
        );
